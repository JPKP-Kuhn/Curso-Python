...
class PygameCtrl(CA.CaixaAcoplada, Basics, SurfScreen, SurfDados, SurfCaixa ): 
	...
	def _pygame_loop(self):
		while True:
			...
			self.Screen.fill(self.CorFundo)	#Evita que as imagens do mouse na tela fiquem estaticas no frame
			...
			if((pyg.mouse.get_pos()[0]!=0)and(pyg.mouse.get_pos()[1]!=0)):
				if (self.MouseFlag ==True): 
					self.Screen.blit(self.MouseImage1,  (pyg.mouse.get_pos()[0]-7,pyg.mouse.get_pos()[1]+3) )
				else: 	
					self.Screen.blit(self.MouseImage2, (pyg.mouse.get_pos()[0],pyg.mouse.get_pos()[1])  )
