...
class PygameCtrl(CA.CaixaAcoplada):
	def __init__(self): 
		...
		self.clock = pyg.time.Clock()	#aviso ao pygame que vamos ter um relogio.
		self.ContTempoSeg = 0
	...
	def _dados_Tela(self): 
		...
		#Criação do relogio
		self.RetanguloRelogio = pyg.draw.rect(self._Screen, (255,255,255), pyg.Rect((410,405),(240,40)))
		FontTempoOperacao = pyg.font.SysFont("Arial", 15)
		self.ContTempoAcumulado+=self.tick
		self.ContTempoSeg = self.ContTempoAcumulado/1000	#Marca tempo em segundos.
		self.TempoMin =1
		self.TempoSeg =1
		TextoTempo = "{0:02d}:{1:02d}".format(int(self.ContTempoSeg), int(self.ContTempoSeg))
